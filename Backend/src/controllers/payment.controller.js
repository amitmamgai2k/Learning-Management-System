...
export const cancelSubscription = asyncHandler(async (req, res, next) => {
    ...
    // Checking the user role
    if (user.role === "ADMIN") {
        return next(new ApiError("Admin does not need to cannot cancel subscription", 400));
    }
    ...
    if (refundPeriod <= timeSinceSubscribed) {
        return next(new ApiError("Refund period is over, so there will not be any refunds provided.", 400));
    }
